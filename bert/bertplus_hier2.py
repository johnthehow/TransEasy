...
class attentions_noclssep_scale_linear_reduced:
	def __init__(self, attentions_noclssep_scale_linear_raw, tokens):
		...
		self.matrices = self.reduced_attention_noclssep_linscale(tokens.pre.spaced)
	...
	def viz(self, lay, head):
		...
		ax.set_xticks(range(data.shape[0]))
		ax.set_yticks(range(data.shape[1]))
		xticklabels = [str(i+1)+' '+j for i,j in zip(range(data.shape[0]), self._tokens.pre.spaced)]
		yticklabels = [j+' '+str(i+1) for i,j in zip(range(data.shape[0]), self._tokens.pre.spaced)]
		ax.set_xticklabels(xticklabels, rotation=45, ha='left')
		ax.set_yticklabels(yticklabels)
...
sent = 'The salesman gave us a demo of the Huggingface course, and it is seemmingly working very well.'
